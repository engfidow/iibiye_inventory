...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchTotalActiveProducts = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/products/data/totalactive");
    ...
    const fetchLastProducts = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/products/fourproducts/last");
