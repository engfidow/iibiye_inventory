...
function UserManagementTable() {
  ...
  const fetchUsers = async () => {
    setLoading(true);
    try {
      const response = await axios.get('https://iibiye-inventory.onrender.com/api/users/getall');
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      if (btnSave) {
        await axios.post('https://iibiye-inventory.onrender.com/api/users/signup', formData, {
          headers: {
        ...
        const updateUrl = `https://iibiye-inventory.onrender.com/api/users/${selectedUser._id}`;
  ...
  const handleEdit = async (index) => {
    ...
    try {
      const response = await axios.get(`https://iibiye-inventory.onrender.com/api/users/${selectedUser._id}`);
      ...
      setImagePreview(`https://iibiye-inventory.onrender.com/${userData.image}`);
    } catch (error) {
  ...
  const handleDelete = async (index) => {
    const selectedUser = users[index];
    try {
      const deleteUrl = `https://iibiye-inventory.onrender.com/api/users/${selectedUser._id}`;
  ...
  const columns = [
    {
      ...
      options: {
        customBodyRender: (value) => {
          return <img src={value ? `https://iibiye-inventory.onrender.com/${value}` : 'https://st3.depositphotos.com/9998432/13335/v/450/depositphotos_133352156-stock-illustration-default-placeholder-profile-icon.jpg'}  alt="User" style={{ height: '50px' }} />;
        },
      },
    },
